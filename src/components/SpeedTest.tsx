...
export default function SpeedTest() {
  ...
  const runSpeedTest = async () => {
    ...
    try {
      ...
      const pingFactor =
        speedTestConfig.pingCalibrationData[1].actualValue /
        speedTestConfig.pingCalibrationData[1].measuredValue;

      const calibratedPing = Math.round(
        calibratePing(measuredPing, pingFactor, speedTestConfig.minPing),
      );
    ...
    const effectiveDownloadFactor =
      downloadSpeed && rawDownloadSpeed
        ? downloadSpeed / rawDownloadSpeed
        : null;
    const effectiveUploadFactor =
      uploadSpeed && rawUploadSpeed ? uploadSpeed / rawUploadSpeed : null;
    ...
    <div className="max-w-lg mx-auto bg-white p-6 rounded-lg shadow-md">
      <div className="mb-6 text-center">
        ...
        <p className="text-gray-600">
          Test your connection's ping, download, and upload speeds
        </p>
      </div>
      ...
      {showAdvanced && testPhase === "complete" && (
        <div className="mb-6 bg-gray-50 p-4 rounded-lg text-sm">
          ...
          <p className="text-gray-600 mb-3">
            Speed test results are calibrated to better match real-world speeds
            using adaptive scaling based on multiple connection profiles.
          </p>
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            <div>
              ...
              <p className="text-xs text-gray-500">
                {speedTestConfig.useAdaptiveScaling
                  ? "Using adaptive calibration"
                  : "Fixed calibration"}
              </p>
            </div>
            <div>
              ...
              <p className="text-xs text-gray-500">
                Server capacity ceiling: {speedTestConfig.serverCapacityCeiling}{" "}
                Mbps
              </p>
            </div>
            <div>
              ...
              <p className="text-xs text-gray-500">
                Based on {speedTestConfig.uploadCalibrationData.length}{" "}
                connection profiles
              </p>
            </div>
          </div>
            ...
            <div className="mt-2 overflow-x-auto">
              <div className="mb-2">
                <p className="font-medium">
                  Download Calibration Reference Points:
                </p>
                <ul className="list-disc ml-4">
                  {speedTestConfig.downloadCalibrationData.map(
                    (point, index) => (
                      <li key={index}>
                        Measured: {point.measuredValue} Mbps → Actual:{" "}
                        {point.actualValue} Mbps (Factor:{" "}
                        {(point.actualValue / point.measuredValue).toFixed(2)}x)
                      </li>
                    ),
                  )}
                </ul>
              ...
              <div className="mb-2">
                <p className="font-medium">
                  Upload Calibration Reference Points:
                </p>
                <ul className="list-disc ml-4">
              ...
              <div>
                <p className="font-medium">
                  Ping Calibration Reference Points:
                </p>
                <ul className="list-disc ml-4">
                  {speedTestConfig.pingCalibrationData.map((point, index) => (
                    <li key={index}>
                      Measured: {point.measuredValue} ms → Actual:{" "}
                      {point.actualValue} ms (Factor:{" "}
                      {(point.actualValue / point.measuredValue).toFixed(3)}x)
                    </li>
                  ))}
                </ul>
              </div>

...
export default function TypewriterEffect({
  ...
  delay = 1500,
  className = '',
  cursorClassName = 'border-r-2 border-orange-500 ml-1 animate-blink h-4 md:h-5 inline-block relative top-[0.15em]',
  repeat = false,
  onComplete,
    ...
    const handleTyping = () => {
      ...
      if (phaseRef.current === 'typing') {
        if (displayText.length < currentTextRef.current.length) {
          setDisplayText(currentTextRef.current.slice(0, displayText.length + 1));
  ...
  }, [displayText, inView, isComplete, onComplete, repeat, speed, delay, textIndex, texts]);

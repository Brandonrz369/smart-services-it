...
export default function FormDebugger() {
  const [formEndpoint, setFormEndpoint] = useState('https://formspree.io/f/xzzeddgr');
  ...
  // Update a field
  const updateField = (index: number, field: 'name' | 'value', newValue: string) => {
  ...
  const addLog = (message: string) => {
    setSubmissionResult(prev => ({
      ...prev,
      logs: [...prev.logs, `${new Date().toISOString().substring(11, 19)} - ${message}`]
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      addLog(`Starting form submission to ${formEndpoint} using ${formMethod} method`);
      ...
      addLog(`Error during submission: ${error instanceof Error ? error.message : String(error)}`);
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={formMethod}
                    onChange={(e) => setFormMethod(e.target.value as 'json' | 'formdata')}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                  >
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`flex-1 py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${
                  submissionResult.status === 'submitting' ? 'opacity-75 cursor-not-allowed' : ''
                }`}
              >
                {submissionResult.status === 'submitting' ? 'Submitting...' : 'Test Submission'}
              </button>
        ...
        <div className="col-span-1">
          ...
          <div className="h-60 overflow-y-auto bg-gray-50 border border-gray-200 rounded p-3 mb-4 font-mono text-xs">
            {submissionResult.logs.length === 0 ? (
              <p className="text-gray-500 italic">No logs yet. Submit the form to see debug information.</p>
            ) : (
          ...
          <div className={`p-3 border rounded-md mb-4 ${
            submissionResult.status === 'success' ? 'bg-green-50 border-green-200' :
            submissionResult.status === 'error' ? 'bg-red-50 border-red-200' :
            'bg-gray-50 border-gray-200'
          }`}>
            <p className="font-medium mb-2">
              Status: {
                submissionResult.status === 'idle' ? 'Ready' :
                submissionResult.status === 'submitting' ? 'Submitting...' :
            ...
            {(submissionResult.response || submissionResult.error) && (
              <pre className="whitespace-pre-wrap text-xs overflow-x-auto">
                {JSON.stringify(submissionResult.response || submissionResult.error, null, 2)}
              </pre>
            )}
          </div>
        </div>

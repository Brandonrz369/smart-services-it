...
export default function AnalyticsDashboard() {
      ...
      if (eventsStr) {
        try {
          ...
          const filteredEvents = events.filter(
            (event: any) => new Date(event.timestamp) > cutoffTime,
          );
          ...
          filteredData = {
            ...
            firstEventTime:
              filteredEvents.length > 0
                ? new Date(filteredEvents[filteredEvents.length - 1].timestamp)
                : null,
            lastEventTime:
              filteredEvents.length > 0
                ? new Date(filteredEvents[0].timestamp)
                : null,
          };
        } catch (e) {
  ...
  const getChartData = () => {
    ...
    const actionsByTool = selectedTool
      ...
      {!filteredData ? (
        <div className="text-center p-12 bg-gray-50 rounded-lg">
          <p className="text-gray-600">
            No usage data available. Start using tools to track analytics.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <p className="text-3xl font-bold">
                {Object.keys(filteredData.toolCounts).length}
              </p>
              <p className="text-sm text-gray-500 mt-2">
                Most popular:{" "}
                {Object.entries(filteredData.toolCounts).sort(
                  (a, b) => b[1] - a[1],
                )[0]?.[0] || "N/A"}
              </p>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <p className="text-lg font-bold">
                {formatDate(filteredData.lastEventTime)}
              </p>
              <p className="text-sm text-gray-500 mt-2">
                Latest action:{" "}
                ...
                <div>
                  {chartData.toolDistribution.map((item) => (
                    <div
                      ...
                      onClick={() =>
                        setSelectedTool(
                          selectedTool === item.tool ? null : item.tool,
                        )
                      }
                    >
                      <div className="flex justify-between items-center mb-1">
                        <div className="flex items-center">
                          ...
                          ></div>
                          <span
                            className={`font-medium ${selectedTool === item.tool ? "text-blue-600" : ""}`}
                          >
                            {item.tool}
                          </span>
                        </div>
            ...
            <div className="lg:col-span-3 bg-white rounded-lg shadow-md p-6">
              <h3 className="text-lg font-medium mb-4">
                {selectedTool
                  ...
                  : "Select a tool to see actions"}
              </h3>

              {selectedTool &&
              chartData &&
              chartData.actionsByTool.length > 0 ? (
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            {Math.round(
                              (item.count /
                                (filteredData.toolCounts[selectedTool] || 1)) *
                                100,
                            )}
                            % of {selectedTool} usage
                          </p>
                        </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {typeof window !== "undefined" &&
                    localStorage.getItem("toolUsageEvents") &&
                    (() => {
                      try {
                  ...
                  {(!localStorage.getItem("toolUsageEvents") ||
                    JSON.parse(localStorage.getItem("toolUsageEvents") || "[]")
                      .length === 0) && (
                    <tr>
                      <td
                        colSpan={4}

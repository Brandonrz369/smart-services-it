...
const SUBMISSIONS_LOG_FILE = path.join(process.cwd(), 'form-submissions-log.json');

// Initialize log file if it doesn't exist
try {
  if (!fs.existsSync(SUBMISSIONS_LOG_FILE)) {
    fs.writeFileSync(SUBMISSIONS_LOG_FILE, JSON.stringify({ 
      submissions: [],
      stats: {
        total_submissions: 0,
...
function logFormSubmission(formName: string, formData: any, success: boolean, status: number, response: any) {
  try {

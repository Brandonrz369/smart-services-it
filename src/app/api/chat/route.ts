...
Services offered:
1. Computer and Laptop Services ($75-150/hour) {/* Changed */}
   - Hardware diagnostics and configuration {/* Changed */}
   - Software troubleshooting
   - Security cleanup assistance {/* Changed */}
   - Data accessibility consultation {/* Changed */}
   - Screen assessment & options {/* Changed */}
   - Hardware upgrades
...
3. Mobile Device Assistance {/* Changed */}
   - Screen and glass solutions {/* Changed */}
   - Battery replacement
   - Charging port assistance {/* Changed */}
   - Data accessibility consultation and transfer {/* Changed */}
...
5. Data Solutions & Backup (Starting $149) {/* Changed */}
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `You are an AI assistant for LB Computer Help, an IT support and computer services business. {/* Changed */}
...
function getMockResponse(input: string) {
  ...
  if (
    ...
    return "Our pricing depends on the specific services you need. For computer services, we typically charge between $75-$150/hour depending on complexity. For managed IT services, we offer plans starting at $299/month."; // Changed repairs -> services
  }
  ...
  if (lowerInput.includes("services") || lowerInput.includes("offer")) {
    return "We offer computer services, managed IT services, network setup, data accessibility consultation, mobile device assistance, and cloud solutions. Is there a specific service you'd like to know more about?"; // Changed repair -> services, data recovery -> data accessibility consultation, mobile device repair -> mobile device assistance
  }

  // Removed the specific check for "repair" or "fix" as it's too broad and potentially non-compliant
...
export async function POST(req: NextRequest) {
  try {
    console.log(
      "Chat API called with OpenAI key:",
      OPENAI_API_KEY ? "Key is present" : "No API key",
    );
    ...
    if (message === "test") {
      ...
      return NextResponse.json(
        { message: "Chatbot API is working correctly. Test successful!" },
        { status: 200 },
      );
    }

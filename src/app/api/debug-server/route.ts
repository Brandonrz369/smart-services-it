...
try {
  if (!fs.existsSync(DEBUG_LOG_FILE)) {
    fs.writeFileSync(DEBUG_LOG_FILE, JSON.stringify({ 
      logs: [],
      server_info: {
    ...
    if (action === 'log') {
      ...
      if (data?.filter) {
        ...
        if (filter.search) {
          ...
          logs.logs = logs.logs.filter((log: any) => 
      ...
      try {
        addLogEntry('info', 'Testing FormSpree connectivity', { endpoint: 'https://formspree.io/f/xzzeddgr' });
        ...
        addLogEntry('error', 'FormSpree test error', { error: error instanceof Error ? error.message : String(error) });
        return NextResponse.json({ 
          success: false, 

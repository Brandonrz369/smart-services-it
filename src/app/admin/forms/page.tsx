...
export default function FormsMonitor() {
  ...
  const [formSubmissions, setFormSubmissions] =
    useState<FormSubmissionLogs | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [runningTests, setRunningTests] = useState(false);
  const [activeTab, setActiveTab] = useState<"tests" | "submissions">(
    "submissions",
  );
  ...
  const fetchTestResults = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError(
          `Error fetching test results: ${response.status} ${response.statusText}`,
        );
      }
    } catch (err) {
      setError(
        `Error fetching test results: ${err instanceof Error ? err.message : String(err)}`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const runTests = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError(
          `Error running tests: ${response.status} ${response.statusText}`,
        );
        setRunningTests(false);
      }
    } catch (err) {
          ...
          <div className="p-4 bg-gray-50 border-b border-gray-200">
            ...
            {formSubmissions?.stats?.last_submission && (
              <p className="text-sm text-gray-600">
                Last submission:{" "}
                {formatDate(formSubmissions.stats.last_submission)}
              </p>
            )}
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-blue-600">
                    {formSubmissions.stats.total_submissions}
                  </p>
                </div>
                <div className="bg-green-50 p-4 rounded-lg">
                  <p className="text-sm text-gray-500">Successful</p>
                  <p className="text-2xl font-bold text-green-600">
                    {formSubmissions.stats.successful_submissions}
                  </p>
                </div>
                <div className="bg-red-50 p-4 rounded-lg">
                  <p className="text-sm text-gray-500">Failed</p>
                  <p className="text-2xl font-bold text-red-600">
                    {formSubmissions.stats.failed_submissions}
                  </p>
                </div>
              </div>
            )}
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            <p className="text-sm">
              Test individual forms by clicking the links below:
            </p>
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md text-sm">
              ...
              <p className="text-yellow-700">
                For advanced debugging, use the Python debug client on your
                server:
                <br />

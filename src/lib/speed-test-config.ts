 ...
 */
function findNearestDataPoints(value: number, dataPoints: CalibrationDataPoint[]) {
  // Sort data points by measured value
  const sortedPoints = [...dataPoints].sort((a, b) => a.measuredValue - b.measuredValue);
  ...
  for (let i = 0; i < sortedPoints.length - 1; i++) {
    if (value >= sortedPoints[i].measuredValue && value <= sortedPoints[i + 1].measuredValue) {
      return {
...
function interpolate(
  ...
  const percentage = (value - lower.measuredValue) / 
    (upper.measuredValue - lower.measuredValue);
  
  // Interpolate between the lower and upper actual values
  return lower.actualValue + percentage * (upper.actualValue - lower.actualValue);
}
...
export function calibrateSpeed(
  ...
  // For download speeds
  if (calibrationFactor === speedTestConfig.downloadCalibrationData[2].actualValue / 
  ...
  // For upload speeds
  else if (calibrationFactor === speedTestConfig.uploadCalibrationData[2].actualValue / 

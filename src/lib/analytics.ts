...
export const trackToolUsage = (tool: string, action: string, metadata?: Record<string, any>) => {
...
export const getToolUsageStats = () => {
  ...
  try {
    ...
    return {
      ...
      toolCounts,
      actionCounts,
      firstEventTime: events.length > 0 ? new Date(events[events.length - 1].timestamp) : null,
